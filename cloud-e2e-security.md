...
This tutorial walks you through key security services available in the {{site.data.keyword.cloud}} catalog and how to use them together. A file sharing application that allows sharing of uploaded files via expiring links serves as sample scenario.
{:shortdesc}
...
1. Go into the IBM Cloud catalog and create an instance of [{{site.data.keyword.cloudaccesstrailshort}}](https://console.bluemix.net/catalog/services/activity-tracker). Note that there can only be one instance of {{site.data.keyword.cloudaccesstrailshort}} per space - Activity Tracker does not use resource group provisioning.
...
1. On the left-hand side menu of the {{site.data.keyword.keymanagementserviceshort}} service dashboard page under "Getting Started", click the **Manage** menu item for the service. Press the **Add Key +** button to create a new key in the service. This new root key will be used to encrypt the storage bucket content.
...
1. On the left-hand side menu of the {{site.data.keyword.cos_short}} service dashboard page, click the **Service credentials** menu item and click the **New credential +** button to create a new access key for the service.
   ...
   * Make note of the credentials (use **View credentials**) - you will need them in a later step.
1. Under the **Endpoint** menu item, set **Select resiliency** to **Regional**, set the **Select location** to the target region you chose earlier. Then, below **Select resiliency** and **Select location** and under the **Public** banner, write down the Public service URL endpoint. It will be used later in the configuration of the application.
...
1. Go to [Identity & Access > Authorizations](https://console.bluemix.net/iam/#/authorizations) page in the {{site.data.keyword.cloud_notm}} console.
1. Click the **Create +** button to create a new authorization.
...
Finally create the IBM Cloud Object Storage s3 bucket.

1. From the IBM Cloud Dashboard page under the **Services** section, click on the **secure-file-storage-cos** service to bring up its' dashboard page.
1. Click the **Create bucket +** button.
   1. In the **Create bucket** section, set the **Unique bucket name** to a unique value, such as **&lt;your-initials&gt;-secure-file-upload**.
   1. Set **Resiliency** to **Regional**.
   1. Set **Location** to the same region you selected where you created the **secure-file-storage-kp** service.
   1. Set **Storage class** to **Standard**.
1. Check the **Add Key Protect Keys** radio button.
   1. Select the **secure-file-storage-kp** service.
   1. Select **secure-file-storage-root-enckey** as the key.
1. There is no need to click the optional **Add archive policy** radio button.
1. Click the **Create bucket** button to finish with the new bucket.

#### A database to map relationships between users and their uploaded files in ICOS.

A {{site.data.keyword.cloudant_short_notm}} database will store an individual JSON document with relevant metadata for every file uploaded via the tutorial application.

1. Create an instance of [{{site.data.keyword.cloudant_short_notm}}](https://console.bluemix.net/catalog/services/cloudantNoSQLDB).
   * Set the **Service name:** to **secure-file-storage-cloudant**.
   * Set the **Choose a region/location to deploy in:** to the same region used with the other services.
   * Use the same resource group for **Select a resource group:** as for the previous services.
   * Set **Available authentication methods** to **Use only IAM**. No need to use legacy methods for this tutorial.
   * Under the **Pricing Plans** section, select the **Lite** pricing plan.
   * Click the **Create** button to provision the new Cloudant service.
1. On the left-hand side menu of the new service's dashboard page, under the **Service credentials** menu item, click the **New credential** button to allocate a new Cloudant access key.
   * In the **Add new credential** dialog, set the **Name** to **secure-file-storage-cloudant-acckey**.
   * Set **Role** to **Manager**
   * Keep the default values for the *Optional* fields
   * Click the **Add** button to create the new credential.
1. Make note of the credentials (use **View credentials**) - you will need them in a later step.
1. Under **Manage**, launch the Cloudant dashboard.
1. Using the **Create Database** menu item on the right, create a database named **secure-file-storage-metadata**.
...
1. Create an instance of [{{site.data.keyword.appid_short}}](https://console.bluemix.net/catalog/services/AppID).
   ...
   * Under the **Pricing Plans** section, select the **Lite** pricing plan.
   * Click the **Create** button to provision the new AppID service.
1. On the left-hand side menu of the {{site.data.keyword.appid_short}} service dashboard page under the **Identity Providers / Manage** menu item in the **Settings** tab, add a **web redirect URL** pointing to the domain you will use for the application. Assuming your cluster Ingress subdomain is 
_&lt;cluster-name&gt;.us-south.containers.appdomain.cloud_, the redirect URL will be `https://secure-file-storage.`_&lt;cluster-name&gt;.us-south.containers.appdomain.cloud_`/appid_callback`. {{site.data.keyword.appid_short}} requires the web redirect URL protocol to be **https**. You can view your Ingress subdomain in the cluster dashboard (under the **Overview** tab) or from the IBM Cloud Kubernetes service CLI with `ibmcloud ks cluster-get <cluster-name>`. The first part of the redirect **https://** URL (the <hostname> portion referenced in the integration mentioned aove) comes from the $IMAGE_NAME variable in the secure-file-storage.yaml file customized in Step 4 of the **Fill in credentials and configuration settings** section below. 
...
1. Use the IBM Cloud Container Service CLI command `ibmcloud cr info` to retrieve the "Container Registry" value and the CLI command `ibmcloud cr namespaces` command to retrieve your cloud registry namespaces to build the tutorial application Docker image in {{site.data.keyword.registryshort_notm}} using the following CLI command:
   ```sh
